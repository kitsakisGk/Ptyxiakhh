...
def save_plots(history, acc):
	if acc == 1:
		...
		ax.set_title("CNN LSTM Model Accuracy")
		ax.set_ylabel("Accuracy")
		ax.set_xlabel("Epoch")
		ax.legend(["Accuracy", "Validation Accuracy", "Loss", "Validation Loss"])
		fig.savefig('Images/Model_Accuracy_CNN_LSTM.png')
		fig.clf()
		ax.cla()
		del fig, ax
	else:
		...
		fig.savefig('Images/Model_Accuracy_CNN_LSTM_F1.png')
		fig.clf()
		ax.cla()
		del fig, ax
	return
...
if __name__ == '__main__':
	...
	# Set the callbacks.
	checkpoint = ModelCheckpoint("Models/CNN_LSTM.h5", save_weights_only=True, monitor='val_accuracy', mode='max', save_best_only=True)
	early = EarlyStopping(monitor='val_accuracy', min_delta=0, patience=20, verbose=1, mode='auto')

	# Train the model.
	hist = model.fit(tf.expand_dims(x_train, axis=-1), y_train, epochs=25, batch_size=10000, callbacks=[checkpoint, early], verbose=1, validation_split=0.2)
